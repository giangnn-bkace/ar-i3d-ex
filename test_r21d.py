...
_CHECKPOINT_PATHS_RGB = [
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-1', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-2', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-3', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-4', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-5', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-6', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-7', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-8', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-9', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-10', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-11', 'clipped_data_rgb-20'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-rgb-12', 'clipped_data_rgb-20')
]
_CHECKPOINT_PATHS_FLOW = [
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-1', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-2', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-3', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-4', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-5', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-6', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-7', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-8', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-9', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-10', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-11', 'clipped_data_flow-45'),
    os.path.join(_LOG_ROOT, 'finetune-clipped_data-flow-12', 'clipped_data_flow-45')
]
...
def process_video(data_info, name, mode, is_training=False):
    ...
    if is_training:
        ...
        clip_seq, label_seq = data.get_element(1, _CLIP_SIZE, frame_size=_FRAME_SIZE)
...
#        os.path.join('/data1/yunfeng/i3d_test/data', dataset, 'testlist%02d' % split+'.txt'))
    ...
    with tf.variable_scope(_SCOPE['flow']):
        # insert i3d model
        flow_model = r2plus1d.R2Plus1D()
        # the line below outputs the final results with logits
        # __call__ uses _template, and _template uses _build when defined
        flow_logits = flow_model(flow_clip_holder, is_training=is_train_holder)
        flow_logits_dropout = tf.nn.dropout(flow_logits, dropout_holder)
        # To change 400 classes to the ucf101 or hdmb classes
        flow_fc_out = tf.layers.dense(flow_logits_dropout, _CLASS_NUM[dataset], use_bias=True)
    ...
    for variable in tf.global_variables():
        ...
        #print(variable.name)
        if tmp[0] == _SCOPE['rgb']:
            variable_map[variable.name.replace(':0', '').replace('RGB/', '')] = variable
    ...
    for variable in tf.global_variables():
        ...
        if tmp[0] == _SCOPE['flow']:
            variable_map[variable.name.replace(':0', '').replace('Flow/', '')] = variable
